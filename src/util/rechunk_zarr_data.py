...
def _process_exp(replicate: str, exp: str) -> None:
    if "detection" in root[replicate][exp]:
        ...
        # del root[f"{replicate}/{exp}/detection"]
    ...
    if "linking" in root[replicate][exp]:
        ...
        # del root[f"{replicate}/{exp}/linking"]
...
def _cleanup_dirs(replicate: str, exp: str) -> None:
    ...
    if os.path.isdir(linking_path):
        os.rmdir(linking_path)
...
# Parallel(n_jobs=-1)(delayed(_cleanup_dirs)(replicate, exp) for replicate, exp in tqdm(exp_data))
